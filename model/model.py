...
class DecoderLayerStack(nn.Module):
    ...
    def forward(self, x, cond, t,style):
        for layer in self.stack:
            x = layer(x, cond, t,style)
        return x
...
class DanceDecoder(nn.Module):
    def __init__(
        ...
        self.cond_projection = nn.Linear(cond_feature_dim-self.genre_dim, latent_dim)
        self.genre_projection = nn.Linear(self.genre_dim,latent_dim)
    ...
    def forward(
        ...
        genre = cond_embed[:, :, -512:]
        cond_embed = cond_embed[:, :, :-512]

        genre = genre[:,0,:]
        genre_embed = self.genre_projection(genre)
        ...
        c = torch.cat((cond_tokens, t_tokens),
                      dim=-2)
        cond_tokens = self.norm_cond(
            c)

        # Pass through the transformer decoder
        # attending to the conditional embedding
        output = self.seqTransDecoder(x, cond_tokens, t, style=genre_embed)
