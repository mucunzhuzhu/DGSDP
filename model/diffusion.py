...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise): # predict_start_from_noise
        """
    ...
    def long_ddim_sample(self, shape, cond, **kwargs):
        ...
        times = list(reversed(times.int().tolist())) # [999,979,959,...19,-1]
    ...
    def render_sample(
        ...
        print('self.guidance_weight',self.guidance_weight)
        ...
        if mode == "long":
            ...
            '''
            skeleton_render(
                full_pose[0], #{Tensor:(900,24,3)}
                epoch=f"{epoch}",
                out=render_out, # 'renders/'
                name=name, # {list:11}
                sound=sound, # True
                stitch=True,
                sound_folder=sound_folder, # ood_sliced
                render=render # True
            )
            '''
            if fk_out is not None:
                outname = f'{epoch}_{"_".join(os.path.splitext(os.path.basename(name[0]))[0].split("_"))}.pkl'
